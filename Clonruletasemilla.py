...
st.title("🔐 Simulador de Clonación de Semilla de Ruleta")
st.markdown("Este simulador imita cómo un atacante podría intentar descubrir la semilla de una ruleta electrónica.")
...
entrada = st.text_input(
    "Ingresá una secuencia de entre 3 y 7 números separados por espacio:",
    "2 25 33 30 31"
)

rango_max = st.slider(
    "Máximo de semillas a probar:",
    min_value=1000,
    max_value=500000,
    value=50000,
    step=1000
...
if entrada:
    try:
        observados = list(map(int, entrada.strip().split()))
